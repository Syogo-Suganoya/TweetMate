...
def get_user_posts_mock(post_id=None):
    """
    get_user_posts のモックデータ。
    post_id を指定すると、そのIDのポストのみを返す。
    指定しない場合は全ポストを返す。
    """
    ...
    if post_id is not None:
        data = [post for post in data if post.get("id") == post_id]

    posts = [MockPost(datetime.strptime(post["created_at"], "%Y-%m-%d %H:%M:%S"), post["text"]) for post in data]
